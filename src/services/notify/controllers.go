...
func (s *Service) postTemplateMsg(ctx iris.Context) {
	...
	req := base.TemplateMsg{}

	err := ctx.ReadJSON(&req)
	if err != nil {
		ctx.StatusCode(iris.StatusBadRequest)
		_, _ = ctx.Write(sptty.NewRequestError(base.ErrReqNotify, err.Error()))
		return
	}
...
func (s *Service) postMPTemplateMsg(ctx iris.Context) {
	...
	if err != nil {
		ctx.StatusCode(iris.StatusBadRequest)
		_, _ = ctx.Write(sptty.NewRequestError(base.ErrMsgNotify, err.Error()))
		return
	}
